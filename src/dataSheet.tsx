...
const ArrowLeft = (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m12 19-7-7 7-7" /><path d="M19 12H5" /></svg>;
const ArrowRight = (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m12 5 7 7-7 7" /><path d="M5 12h14" /></svg>;
...
const arrayToJson = (header: RowType, rows: RowType[]) =>
  rows.map((row) =>
    header.reduce((acc, col, i) => { acc[col] = row[i]; return acc; }, {} as Record<string, any>)
  );
...
const DataSheet = () => {
  ...
  const filteredBody = useMemo(() => {
    if (!debouncedSearch) return cleanedBody;
    const lower = debouncedSearch.toLowerCase();
    return cleanedBody.filter(row => row.some(cell => cell !== null && String(cell).toLowerCase().includes(lower)));
  }, [cleanedBody, debouncedSearch]);
  ...
  useEffect(() => setPage(1), [searchTerm, totalPages]);
  ...
  const handleSortClick = (colIndex: number) => {
    ...
    if (newMode !== "original" && newMode !== null) {
      const sorted = sortCSVRows(originalArray.slice(1), colIndex, newMode);
    ...
    setCSVData({
      ...
      content: {
        array: newArray,
        json: arrayToJson(newArray[0], newArray.slice(1)),
      },
    });
    setPage(1);
  };
        ...
        <table className="min-w-full border-collapse table-auto">
          <thead className="bg-gray-700/80 backdrop-blur-sm sticky top-0 z-10 shadow-md">
            <tr>
              ...
              {header.map((cell, i) => (
                <th key={i} onClick={() => handleSortClick(i)} className="group relative cursor-pointer border-b border-gray-600 px-4 py-3 text-left text-sm font-semibold sticky top-0 bg-gray-700/80 text-gray-300 transition duration-150 hover:bg-gray-600/80" style={{ minWidth: "140px" }}>
                  <div className="flex items-center justify-between">
                    ...
                    {/* Hamburger moved outside flex for no overlap */}
                    <div className="ml-2">
                      <button onClick={e => { e.stopPropagation(); setActiveFilterCol(i === activeFilterCol ? null : i); }}
                        className="flex items-center justify-center hover:bg-gray-600 rounded-full p-1">
                        <HamburgerIcon className="w-4 h-4 text-white" />
                      </button>
                    </div>
                  </div>
                </th>
          ...
          <tbody>
            {paginatedRows.map((row, ri) => (
              <tr key={ri} className="transition duration-150 hover:bg-blue-900/20 odd:bg-gray-900/50 even:bg-gray-800/50">
                ...
                {row.map((cell, ci) => (
                  <td key={ci} className="border-b border-gray-700 px-4 py-2 text-sm break-words max-w-xs text-gray-300">{cell ?? <span className="text-gray-500 italic">NULL</span>}</td>
                ))}
              </tr>
            ))}
